...
function pbxFile(filepath, opt) {
    ...
    // for embed frameworks
    if (opt.embedFramework == true) {
        this.group = 'Embed Frameworks';
        this.addAttributes('CodeSignOnCopy', 'RemoveHeadersOnCopy');
    } else {
    ...
    if (opt.weak === true) this.addAttributes('Weak');
    if (opt.compilerFlags) this.addCompilerFlags(opt.compilerFlags);
}

pbxFile.prototype.addAttributes = function() {
    this.settings = this.settings || {};
    this.settings.ATTRIBUTES = this.settings.ATTRIBUTES || [];
    Array.prototype.push.apply(this.settings.ATTRIBUTES, arguments);
}

pbxFile.prototype.addCompilerFlags = function(flags) {
    this.settings = this.settings || {};
    var compilerFlags = unquoted(this.settings.COMPILER_FLAGS || '""');
    if (compilerFlags.length > 0) compilerFlags.concat(' ');
    compilerFlags.concat(flags);
    this.settings.COMPILER_FLAGS = '"' + compilerFlags + '"';
}
